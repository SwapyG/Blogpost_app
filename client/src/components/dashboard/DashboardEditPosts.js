...
import { getPostById, updatePost } from "../../redux/features/posts/postSlice";
...
const DashboardEditPost = () => {
  ...
  const { tags, isLoading: isTagsLoading } = useSelector((state) => state.tags);
  ...
  useEffect(() => {
    if (currentPost) {
      ...
      if (currentPost.featuredImage) {
        setPreviewImage(currentPost.featuredImage);
      }
    }
  }, [currentPost]);

  // Handle success or error from post update
  useEffect(() => {
    if (isError) {
      toast.error(message);
  ...
  }, [isError, isSuccess, message, navigate, isSubmitting]);
  ...
  if (isLoading || isCategoriesLoading || isTagsLoading || !currentPost) {
    return <Spinner />;
  }
